...
Запуск выполняется командой `docker-compose up`

### Лабораторная 4

gosu - это утилита командной строки для Linux, предназначенная для выполнения команд от имени другого пользователя

gosu не требует ввода пароля

Основным вариантом использования gosu является переход от root к непривилегированному пользователю во время запуска контейнера (обычно в точке входа)

Использовать gosu необходимо для того, чтобы избежать запуска контейнера под рутом (root) из-за проблем с безопасностью, т.к. root имеет все права доступа

#### Доработать Dockerfile для использования скрипта
- [ ] Создание непривилегированного пользователя в образе
- [ ] Установка gosu для переключения пользователя при запуске контейнера
- [ ] Создание entrypoint-скрипта, который будет запускать приложение от имени созданного непривилегированного пользователя


`groupadd -r myuser` создает группу с именем myuser.
`useradd -r -g myuser myuser1` создает пользователя с именем myuser1 и добавляет его в группу myuser. Флаги -r делают пользователя системным (непривилегированным).

#### Запуск

`docker run -p 8000:8000 server-app python server.py`

Команда cat /etc/passwd в Linux выводит содержимое файла /etc/passwd, который содержит информацию о пользователях системы. Каждая строка в этом файле представляет одного пользователя и содержит разделенные двоеточиями поля, включая:

Имя пользователя.
Зашифрованный пароль (обычно x, что означает, что пароль хранится в файле /etc/shadow).
Идентификатор пользователя (UID).
Идентификатор группы (GID).
Поле комментария (обычно содержит полное имя пользователя).
Домашний каталог пользователя.
Оболочка по умолчанию.

### Лабораторная 5

- [ ] Подключение экземпляра БД PostgreSQL в docker-compose
- [ ] Подключение SQLAlchemy для работы с базой данных PostgreSQL
- [ ] Создание модели данных для хранения в БД
- [ ] Определение endpoint-ов для вывода списка пользователей, вывода данных пользователя по его идентификатору, добавления и удаления пользователя из БД
- [ ] Доработка приложения для передачи параметров подключения к БД через аргументы командной строки
...
#### Зависимости
Драйвер для работы с БД PostgreSQL
`pip install psycopg2`

SQLAlchemy - библиотека, реализующая технологию ORM (Object-Relational Mapping), которая связывает базы данных с концепциями объектно-ориентированных языков программирования
`pip install flask_sqlalchemy`

#### Запуск
Для запуска необходимо собрать образ `server-app-db` при помощи команды `docker build -t server-app-db .`

Запустить многоконтейнерное Docker-приложение: `docker-compose up` 

Файл bash-скрипта `entrypoint.sh` выполняет проверку доступности порта БД, после чего запускает скрипты создания базы данных и приложение для работы с ней